...
contract Oracle {
  ...
  function processRequest(Data memory requestData) public {
    ...
    if (dataTypeCheck.status == 602) {
      ...
      if (
        keccak256(bytes(requestData.dataType)) == keccak256(bytes('Numeric'))
      ) {
        numericProcess.createEvent(
          requestIndexLength,
          requestData.question,
          requestData.callBackAddress
        );
      } else if (
        keccak256(bytes(requestData.dataType)) == keccak256(bytes('String'))
      ) {
        stringProcess.createEvent(
          requestIndexLength,
          requestData.question,
          requestData.callBackAddress
        );
  ...
  function checkDataType(
    Data memory _data
  ) public pure returns (Response memory) {
    ...
    if (
      keccak256(dataTypeBytes) != keccak256(numericType) &&
      keccak256(dataTypeBytes) != keccak256(stringType)
    ) {
